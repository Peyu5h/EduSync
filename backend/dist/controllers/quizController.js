...
export const createQuiz = async (req, res) => {
    try {
        const { title, description, courseId, questions, status, startTime, endTime, duration, isAiGenerated, contentForAi, } = req.body;
...
export const updateQuiz = async (req, res) => {
    try {
        const { quizId } = req.params;
        const { title, description, status, startTime, endTime, duration, questions, } = req.body;
        if (!quizId) {
            return res.status(400).json({
                success: false,
                message: "Quiz ID is required",
...
export const getQuizLeaderboard = async (req, res) => {
    try {
        ...
        const leaderboard = attempts.map((attempt, index) => {
            const completionTimeMs = attempt.completedAt && attempt.startedAt
...
export async function generateQuestionsWithAI(content) {
    try {
...
export const getActiveQuizzesByCourse = async (req, res) => {
    try {
        ...
        if (userId) {
            quizzesWithAttempts = await Promise.all(quizzes.map(async (quiz) => {
                const attempt = await prisma.studentQuizAttempt.findFirst({
                    where: {
                        quizId: quiz.id,
                        userId,
    ...
    try {
        ...
        const studentProgress = enrolledStudents.map((student) => {
            ...
            const latestAttempt = studentAttempts.length > 0
                ? studentAttempts.reduce((latest, current) => new Date(current.startedAt) > new Date(latest.startedAt)
                    ? current
                    : latest)
                : null;
            return {
        ...
        const attempted = attempts.filter((attempt, index, self) => index === self.findIndex((a) => a.userId === attempt.userId)).length;
